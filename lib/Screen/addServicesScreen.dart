...
class _AddServicesScreenState extends State<AddServicesScreen> {
  ...
  final TextEditingController serviceSpecialPriceController =
  ...
  addServicesVendor() async {
    ...
    var request = http.MultipartRequest(
        'POST', Uri.parse(ApiServicves.addServicesVendor));
    request.fields.addAll(data);
    debugPrint(
        "=========add Services fieldsss======${request.fields}===========");
    ...
    if (response.statusCode == 200) {
      ...
      Fluttertoast.showToast(
          msg: "${finalResult['message']}, Please activate this service ");
  ...
  updateServicesVendor() async {
    ...
    var request = http.MultipartRequest(
        'POST', Uri.parse(ApiServicves.updateServicesVendor));
    request.fields.addAll(data);
    debugPrint(
        "=========update Services fieldsss======${request.fields}===========");
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      var finalResponse = await response.stream.bytesToString();
  ...
  addDynamicTextFieldsCard() {
    ...
    listDynamicTextFieldsCard.add(DynamicTextFieldsCardWidget(
  ...
  submitServiceData() {
    ...
    debugPrint(
        "service data $serviceData, heading${headingController.text}, serviceNames$serviceNames  prices$prices special prices $specialPrices");
  ...
  void initState() {
    ...
    if (widget.isUpdate) {
      ...
      serviceSpecialPriceController.text =
          widget.service!.specialPrice.toString();
    }
  }
      ...
      bottomSheet: Row(
        ...
        children: [
          ElevatedButton(
              ...
              onPressed: () {
                if (widget.isUpdate &&
                    headingFormKey.currentState!.validate()) {
